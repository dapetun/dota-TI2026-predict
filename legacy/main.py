...
DEPRECATED legacy entrypoint (LightGBM + STRATZ/CSV path).

Canonical CLI:
  - ``python scripts/train_compare.py`` — XGB + CatBoost + blend
  - ``python scripts/export_web_data.py`` — JSON for GitHub Pages UI

Prefer those scripts; this module remains for historical experiments only.
"""
...
warnings.warn(
    "main.py is a deprecated legacy pipeline. Use scripts/train_compare.py + "
...
def step_collect_data(raw_dir: str = "data/raw"):
    """Step 1: Collect data from OpenDota (STRATZ path removed)."""
    print("=" * 60)
    print("STEP 1: Data Collection (legacy)")
    print("=" * 60)
